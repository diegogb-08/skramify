...
const ScrumBoard = () => {
  ...
  return (
    <div className='w-full h-full flex justify-center bg-gray-50 p-4'>
      {
        scrumBoard.columnOrder?.map(columnnId => {
          const column: Column = scrumBoard.columns[columnnId as keyof typeof scrumBoard.columns]
          const tasks: TaskCard[] = column.taskIds.map(taskId => scrumBoard.tasks[taskId as keyof typeof scrumBoard.tasks])
          return <ColumnBoard description={column.description} key={column.id} tasks={tasks} />
        })
      }
    </div>
  )
